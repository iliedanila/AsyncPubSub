...
int main() {
    ...
    for (auto i = 0; i < 1; i++) {
        auto node = std::make_shared<NetworkLayer::Node>(
            "subscriber" + std::to_string(i), ioservice);
        ...
        subscriber->addSubscription(
            subscription, [node](LogicalLayer::SubscriptionData& data) {
    ...
    for (auto i = 0; i < 1; i++) {
        auto node = std::make_shared<NetworkLayer::Node>(
            "publisher" + std::to_string(i), ioservice);
        node->connect("localhost", 7777, false);
        std::set<std::string> publisherIdentity = {
            "MyCustomType1", "MyCustomType2", "MyCustomType3"};
        auto publisher =
            std::make_shared<LogicalLayer::Publisher>(*node, publisherIdentity);
            ...
            post(ioservice, [&] {
                for (auto& subscriber : subscribers) {
                    std::set<std::string> subscription = {"MyCustomType1",
                                                          "MyCustomType2"};
                    subscriber->addSubscription(
                        subscription, [](LogicalLayer::SubscriptionData& data) {
                            std::string receivedData;
                            for (auto& typeData : data.getData()) {
                                receivedData += typeData.first;
                                receivedData += ": ";
